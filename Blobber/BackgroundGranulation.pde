...
class BackgroundGranulation extends Layer {
  ...
  public GranularSamplePlayer player2;
  ...
  void loadFilenames() {
    ...
    java.io.FilenameFilter imgFilter = new java.io.FilenameFilter() {
      boolean accept(File dir, String name) {
        return name.toLowerCase().endsWith(".wav") || name.toLowerCase().endsWith(".aif");
      } 
    };
  ...
  void setSamples() {
    ...
    player2 = new GranularSamplePlayer(ac, SampleManager.sample(audioFile2));

    player2.getLoopEndEnvelope().setValue(SampleManager.sample(audioFile2).getLength());
  }
